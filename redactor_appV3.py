...
using ε=8.0 model for PII detection with color coding.
"""
...
class ModelBasedPIIProcessor:
    ...
    def _get_color_scheme(self):
        """Define color scheme for different PII confidence levels."""
        return {
            # High confidence PII - Bright colors
            'HIGH_CONFIDENCE': {
                'PERSONAL': '#3498db',     # Bright blue
                'CONTACT': '#2ecc71',      # Bright green
                'FINANCIAL': '#e74c3c',    # Bright red
                'GENERAL': '#9b59b6'       # Bright purple
            },
            # Medium confidence PII - Medium colors
            'MEDIUM_CONFIDENCE': {
                'PERSONAL': '#5dade2',     # Medium blue
                'CONTACT': '#52be80',      # Medium green
                'FINANCIAL': '#ec7063',    # Medium red
                'GENERAL': '#af7ac5'       # Medium purple
            },
            # Low confidence PII - Pastel colors
            'LOW_CONFIDENCE': {
                'PERSONAL': '#aed6f1',     # Pastel blue
                'CONTACT': '#abebc6',      # Pastel green
                'FINANCIAL': '#fadbd8',    # Pastel red
                'GENERAL': '#e8daef'       # Pastel purple
            }
        }
    ...
    def extract_features(self, text: str) -> List[Dict]:
        ...
        for i, word in enumerate(words):
            ...
            word_features = [
                ...
                1.0 if word and word[0].isupper() else 0.0,  # Has caps - CORRECTED LINE
            ...
            for i, (feature, prob) in enumerate(zip(features_list, probs_np)):
                ...
                if pii_prob > 0.5:  # Threshold for PII detection
                    ...
                    if matches:
                        ...
                        color = self._get_pii_color(pii_type, pii_prob)
    ...
    def _get_pii_category(self, pii_type: str) -> str:
        """Categorize PII type."""
        categories = {
        ...
        return categories.get(pii_type, 'GENERAL')
    
    def _get_pii_color(self, pii_type: str, confidence: float) -> str:
        """Get color for PII based on type and confidence."""
        category = self._get_pii_category(pii_type)
        ...
        return self.color_scheme[confidence_level].get(category, '#7f8c8d')
        ...
        for pii_type, pattern in pii_patterns.items():
            for match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                pii_entities.append({
                    ...
                    'color': self._get_pii_color(pii_type, confidence),
                    'category': self._get_pii_category(pii_type)
                })
    ...
    def highlight_text(self, text_widget: tk.Text, text: str, pii_entities: List[Dict], font_size: int = 16):
        ...
        # Configure tags for each PII entity
        for entity in pii_entities:
            # Create unique tag name
            tag_name = f"pii_{entity['type']}_{entity['start']}"
            
            # Determine font weight based on confidence
            font_weight = 'bold' if entity['confidence'] > 0.7 else 'normal'
...
class RedactorAppDemoLarge:
    ...
    def create_widgets(self):
        ...
        # Color Legend
        self.create_color_legend(bottom_panel)
    ...
    def create_color_legend(self, parent):
        """Create color legend for confidence levels."""
        legend_frame = ttk.LabelFrame(parent, text="🎨 CONFIDENCE COLOR CODING",
                                     padding=(25, 20))
        legend_frame.pack(side='top', fill='x', pady=(15, 0))
        
        canvas = tk.Canvas(legend_frame, height=150, bg='#1e272e',
                          highlightthickness=0)
        canvas.pack(fill='x')
        
        # Confidence levels
        confidence_levels = [
            ("HIGH CONFIDENCE (≥80%)", "Bright colors, Bold text"),
            ("MEDIUM CONFIDENCE (60-79%)", "Medium colors, Normal text"),
            ("LOW CONFIDENCE (<60%)", "Pastel colors, Normal text")
        ]
        
        # Category colors
        categories = [
            ("PERSONAL", "#3498db", "#5dade2", "#aed6f1"),
            ("CONTACT", "#2ecc71", "#52be80", "#abebc6"),
            ("FINANCIAL", "#e74c3c", "#ec7063", "#fadbd8"),
            ("GENERAL", "#9b59b6", "#af7ac5", "#e8daef")
        ]
        ...
        for level, desc in confidence_levels:
            canvas.create_text(x_pos, y_pos, text=level + ":", 
                             anchor='nw', fill='#ecf0f1', 
                             font=('Segoe UI', self.label_font_size - 2, 'bold'))
            canvas.create_text(x_pos + 250, y_pos, text=desc,
        ...
        for category, high, medium, low in categories:
            # High confidence
            canvas.create_rectangle(x_pos, y_pos, x_pos + 20, y_pos + 20,
                                  fill=high, outline='')
            canvas.create_text(x_pos + 30, y_pos + 10, text=category,
                             anchor='w', fill='#ecf0f1',
                             font=('Segoe UI', self.label_font_size - 2))
            
            # Medium confidence
            canvas.create_rectangle(x_pos + 120, y_pos, x_pos + 140, y_pos + 20,
                                  fill=medium, outline='')
            
            # Low confidence
            canvas.create_rectangle(x_pos + 210, y_pos, x_pos + 230, y_pos + 20,
                                  fill=low, outline='')
    ...
    def upload_file(self):
        ...
        try:
            ...
            self.pii_processor.highlight_text(
    ...
    def redact_text(self):
        ...
        try:
            ...
            # Display redacted text with highlighting
            self.pii_processor.highlight_text(
                self.redacted_text_widget,
                self.redacted_text,
                self.pii_entities,
                self.text_font_size
            )
            ...
            messagebox.showinfo("Redaction Complete", 
                              ...
                              f"Processing Time: {processing_time:.2f}s\n\n"
                              f"High-confidence PII are in bright colors.\n"
        ...
        if file_path:
            try:
                ...
                metadata = (
                    ...
                    f"PII Detected: {len(self.pii_entities)}\n"
                    f"Average Confidence: {avg_confidence:.1%}\n"
                    f"Color Coding: Bright=High, Medium=Medium, Pastel=Low confidence\n"
                    f"==================================================\n\n"
                )
