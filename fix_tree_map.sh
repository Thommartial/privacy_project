#!/bin/bash
# fix_tree_map.sh - Fix deprecated jax.tree_map usage

echo "🔧 Fixing deprecated jax.tree_map usage..."
echo "="*60

# Backup current files
cp src/utils/dp_utils.py src/utils/dp_utils.py.backup3
cp src/training/train_dp_proper.py src/training/train_dp_proper.py.backup4
...
echo "✅ Updated src/utils/dp_utils.py with tree_util.tree_map"

# Fix train_dp_proper.py
...
echo "✅ Updated train_dp_proper.py with tree_util.tree_map"

# Also check if we need to fix other files that might use jax.tree_map
echo ""
echo "🔄 Checking for other files that might use jax.tree_map..."
find . -name "*.py" -type f -exec grep -l "jax.tree_map" {} \;

echo ""
echo "🛠️  Testing the fix again..."
python -c "
import jax
import jax.numpy as jnp
import sys
