#!/bin/bash
# fix_dropout_error.sh - Fix the dropout PRNG error

echo "🔧 Fixing dropout PRNG error..."
echo "="*60

# Backup current file
cp src/training/train_dp_proper.py src/training/train_dp_proper.py.backup2
...
echo "✅ Fixed dropout PRNG error in src/training/train_dp_proper.py"

# Also need to fix the dp_utils.py add_noise_to_grads function
...
echo "✅ Updated src/utils/dp_utils.py"
...
# Test if we can create and use the model
from src.training.train_dp_proper import ProperDPModel, create_train_state

# Create a simple test
rng = jax.random.PRNGKey(42)
model = ProperDPModel(hidden_size=256)
dummy_input = jnp.ones((2, 8))  # batch_size=2, input_dim=8
...
logits_train = model.apply(params, dummy_input, training=True, rngs={'dropout': dropout_rng})
print(f'✅ Training forward pass works: logits shape = {logits_train.shape}')

# Test forward pass (evaluation)
logits_eval = model.apply(params, dummy_input, training=False)
print(f'✅ Evaluation forward pass works: logits shape = {logits_eval.shape}')

print('\\n✅ All tests passed! Ready to train.')
"

echo ""
echo "🚀 Now run the training with:"
