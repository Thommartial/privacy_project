...
with triple-sized text for better visibility on small screens.
"""
...
class PIIProcessor:
    ...
    def _get_color_scheme(self):
        """Define color scheme for different PII types."""
        return {
            ...
            # Digital Identifiers - Purple shades
            'IPV4': '#8e44ad',
            'IPV6': '#9b59b6',
            'MAC_ADDRESS': '#af7ac5',
            'USERNAME': '#bb8fce',
            ...
            # Demographic Information - Orange shades
            'GENDER': '#d35400',
            'JOB_TITLE': '#e67e22',
            'AREA': '#f39c12',
            
            # Geographic Data - Teal shades
            'GPS_COORDINATES': '#148f77',
            'ORDINAL_DIRECTION': '#17a589',
            'COUNTY': '#48c9b0',
            
            # Technical Identifiers - Gray shades
            'USER_AGENT': '#5d6d7e',
            'URL': '#7f8c8d',
            'DEVICE_INFO': '#95a5a6'
        }
                ...
                for match in re.finditer(pattern, text, re.IGNORECASE):
                    ...
                    if not overlap:
                        pii_entities.append({
                            ...
                            'start': match.start(),
                            'end': match.end(),
                            'color': self.color_scheme.get(pii_type, '#7f8c8d')
                        })
            except re.error:
                continue
    ...
    def highlight_text(self, text_widget: tk.Text, text: str, pii_entities: List[Dict], font_size: int = 16):
        ...
        # Configure tags for each PII type
        for entity in pii_entities:
...
class RedactorAppDemoLarge:
    ...
    def init_stats_labels(self):
        ...
        for i, (label, value) in enumerate(stats):
            ...
            setattr(self, f'stat_{label.lower().replace(":", "").replace(" ", "_")}', value_widget)
    
    def create_pii_legend(self, parent):
        """Create PII type color legend with larger elements."""
        legend_frame = ttk.LabelFrame(parent, text="🎨 PII COLOR LEGEND",
                                     padding=(25, 20))  # Larger padding
        legend_frame.pack(side='top', fill='x', pady=(15, 0))
        
        # Create a canvas for the legend - taller
        canvas = tk.Canvas(legend_frame, height=150, bg='#1e272e',  # Taller
        ...
        categories = {
            ...
            "DIGITAL IDs": ["IPV4", "IPV6", "MAC_ADDRESS", "USERNAME", "PASSWORD"],
            "DEMOGRAPHIC": ["GENDER", "JOB_TITLE", "AREA"],
            "GEOGRAPHIC": ["GPS_COORDINATES", "ORDINAL_DIRECTION", "COUNTY"],
            "TECHNICAL": ["USER_AGENT", "URL", "DEVICE_INFO"]
        }
        ...
        for category, pii_types in categories.items():
            ...
            y_pos += 15
            x_pos += 250  # More space between columns
    ...
    def upload_file(self):
        ...
        try:
            ...
            # Update PII count
            self.stat_pii_count.config(text=str(len(self.pii_entities)))
            
            # Highlight original text with LARGE font
            self.pii_processor.highlight_text(self.original_text_widget, 
                                             self.original_text, 
                                             self.pii_entities,
                                             self.text_font_size)
    ...
    def redact_text(self):
        ...
        try:
            ...
            if self.current_epsilon <= 1.0:
                messagebox.showinfo("Redaction Applied", 
                                  f"Applied STRONG privacy redaction (ε={self.current_epsilon}).\n"
                                  f"High privacy protection, some PII may remain for readability.")
            elif self.current_epsilon <= 3.0:
                messagebox.showinfo("Redaction Applied", 
                                  f"Applied MODERATE privacy redaction (ε={self.current_epsilon}).\n"
                                  f"Good balance of privacy and readability.")
            else:
                messagebox.showinfo("Redaction Applied", 
                                  f"Applied LIGHT privacy redaction (ε={self.current_epsilon}).\n"
                                  f"Maximum accuracy, minimal privacy protection.")
    ...
    def clear_all(self):
        ...
        self.stat_pii_count.config(text="0")
